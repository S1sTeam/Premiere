# Premire strict design philosophy

## Why strict design

Premire is a working tool in which people spend long periods reading code, observing agent actions, and making decisions that affect files and processes. The interface must reduce cognitive noise and make system state obvious.
...
Premire supports compact panels, but density must not reduce target size, merge independent actions, or harm scanning.
