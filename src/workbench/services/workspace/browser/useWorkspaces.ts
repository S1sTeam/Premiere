...
export function useProjects() {
  const [projects, setProjects] = useState<Project[]>(() => {
    if (typeof window !== "undefined") {
      try {
        const raw = localStorage.getItem("premire_browser_projects");
        if (raw) {
  ...
  const [activeProject, setActiveProject] = useState<string | null>(() => {
    if (typeof window !== "undefined") {
      try {
        return localStorage.getItem("premire_browser_active_project");
      } catch {}
    }
    return null;
  });
  ...
  const handleAddProject = useCallback(
    (_onProjectChange?: (folder: string | null, projectId: string | null) => Promise<void>) => {
