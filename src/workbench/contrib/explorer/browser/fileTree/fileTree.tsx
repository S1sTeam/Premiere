...
export function ExplorerView({ cwd, onOpenFile, activeFile, revealPath }: ExplorerViewProps): React.ReactElement {
  ...
  function buildMenuItems(c: CtxState): MenuItem[] {
    ...
    if (entry?.isDir || !entry) {
      ...
      if (!entry) {
        items.push({
          label: t("revealExplorer") || "Reveal in File Explorer",
          onClick: () => {
            void fileService.reveal(p);
          },
        });
      }
    }
    ...
    if (entry) {
      ...
      items.push({
        label: t("copyRelativePath") || "Copy Relative Path",
        onClick: () => {
          const rel = p.startsWith(cwd) ? p.slice(cwd.length).replace(/^[/\\]+/, "") : basename(p);
