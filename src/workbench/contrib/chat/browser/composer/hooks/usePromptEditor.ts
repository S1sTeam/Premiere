...
export function usePromptEditor({ t, onInput }: PromptEditorOptions) {
  ...
  const [markdownEnabled, setMarkdownEnabled] = useState<boolean>(
    () => localStorage.getItem("premire_prompt_markdown") !== "false",
  );
  const [showGhostSyntax, setShowGhostSyntax] = useState<boolean>(
    () => localStorage.getItem("premire_prompt_markdown_ghost") === "true",
  );

  useEffect(() => {
    const handleSettingsChange = () => {
      setMarkdownEnabled(localStorage.getItem("premire_prompt_markdown") !== "false");
      setShowGhostSyntax(localStorage.getItem("premire_prompt_markdown_ghost") === "true");
    };
