...
const PROMPT_HISTORY_KEY = "premire_prompt_history";
...
function saveHistory(entries: HistoryEntry[]) {
  try {
    sessionStorage.setItem(PROMPT_HISTORY_KEY, JSON.stringify(entries.slice(0, 50)));
  } catch {}
}
...
export function usePromptHistory({ editorRef, editorText, setEditorText, clearEditor }: PromptHistoryOptions) {
  const [entries, setEntries] = useState<HistoryEntry[]>(() => loadSavedHistory());
  ...
  const commit = useCallback((text: string, attachments: Attachment[]) => {
    setEntries((prev) => {
      const next = prependHistoryEntry(prev, text, attachments);
      saveHistory(next);
      return next;
    });
  }, []);
