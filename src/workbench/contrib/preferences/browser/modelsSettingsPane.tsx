      ...
      {loading ? (
        ...
        <div className="settings__models-table">
          {Array.from(groups.entries()).map(([providerDbId, providerModels]) => {
            ...
            const hasExplicit = providerModels.some(
              (m) => enabledModels.has(`${m.providerDbId}::${m.id}`) || enabledModels.has(m.id),
            );
            return (
                ...
                <div className={`settings__models-list ${isCollapsed ? "settings__models-list--collapsed" : ""}`}>
                  <div className="settings__models-list-inner">
                    {providerModels.map((model) => {
                      ...
                      const isChecked = hasExplicit
                        ? enabledModels.has(compositeKey) || enabledModels.has(model.id)
                        : true;
                      return (
                        <div key={compositeKey} className="settings__model-row">
                          <div className="settings__model-info" style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            <ProviderLogo
                              providerId={getModelProviderId(model.id, model.name, model.providerId)}
