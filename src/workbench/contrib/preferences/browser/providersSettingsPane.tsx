...
export function ProvidersTab({ connected, onEdit, onDisconnect, onCustom, onConnect }: Props) {
  ...
  // Filter out templates that are already connected to prevent infinite duplicates
  const connectedKeys = new Set(
    connected.flatMap((p) => [
      p.id?.toLowerCase(),
      p.baseUrl?.replace(/\/+$/, "").toLowerCase(),
      p.name?.toLowerCase(),
      p.id === "premire" || p.baseUrl?.includes("sysik.mom") ? "premire" : "",
    ]).filter(Boolean),
  );

  const availableTemplates = filteredTemplates.filter((template) => {
    const tplUrl = template.baseUrl?.replace(/\/+$/, "").toLowerCase();
    const tplId = template.id?.toLowerCase();
    const tplName = template.name?.toLowerCase();
    if (connectedKeys.has(tplId)) return false;
    if (tplUrl && connectedKeys.has(tplUrl)) return false;
    if (tplName && connectedKeys.has(tplName)) return false;
    if (template.id === "premire" && connectedKeys.has("premire")) return false;
    return true;
  });
