...
function getDefaultModelForProvider(providerId?: string, baseUrl?: string): string {
...
export function ConnectPopup({
  ...
  const [form, setForm] = useState({
    ...
    baseUrl: editProvider?.baseUrl ?? template?.baseUrl ?? "https://free.sysik.mom/v1",
  ...
  async function handleConnect(): Promise<void> {
    ...
    setBusy(true);
    try {
      const providerId = template?.id || editProvider?.id;
      const model = form.model.trim() || getDefaultModelForProvider(providerId, form.baseUrl);
      await onConnect({ ...form, model });
    } finally {
      setBusy(false);
    }
  }
    ...
    <div className="connect-popup__overlay" onClick={onClose}>
      <div className={`connect-popup${custom ? " connect-popup--custom" : ""}`} onClick={(e) => e.stopPropagation()}>
        ...
        <div
          ...
          role="tabpanel"
          id={`connect-popup-${activeTab}-panel`}
          aria-labelledby={`connect-popup-${activeTab}-tab`}
        >
          {activeTab === "general" && (
            <>
              {custom && (
              ...
              <div className="connect-popup__section">
                <label className="connect-popup__label">{t("apiKeyPlaceholder") || "API Key"}</label>
                <Input
                  containerClassName="connect-popup__input"
