...
export function GeneralSettingsProvider({ children }: { children: React.ReactNode }): React.ReactElement {
    ...
    Promise.all(keys.map(async (key) => [key, await appState.get(SETTINGS_PREFIX + key)] as const)).then((entries) => {
      setGeneral((previous) => {
        ...
        for (const [key, value] of entries) {
          ...
          if (key === "language" && value === "Russian") {
            next.language = "English";
            void appState.set(SETTINGS_PREFIX + key, "English");
            continue;
          }
  ...
  const updateGeneral = useCallback<UpdateGeneral>((key, value) => {
    ...
    if (key === "promptMarkdown") localStorage.setItem("premire_prompt_markdown", String(value));
    if (key === "promptMarkdownGhost") localStorage.setItem("premire_prompt_markdown_ghost", String(value));
