...
function makeActions(t: (key: any) => string, onCommand?: (id: string) => void): ActionItem[] {
  return [
    {
      id: "search-in-code",
      label: t("searchTitle") || "Search in Code",
      shortcut: "Ctrl+Shift+F",
      action: () => onCommand?.("search-in-code"),
    },
    {
      id: "new-session",
      label: t("newSession") || "New Chat Session",
      shortcut: "Ctrl+N",
      action: () => {},
    },
    {
      id: "toggle-file-tree",
      label: t("filesTitle") || "Explorer / File Tree",
      shortcut: "Ctrl+Shift+E",
      action: () => onCommand?.("toggle-file-tree"),
    },
    {
      id: "toggle-git-panel",
      label: t("gitTitle") || "Source Control (Git)",
      shortcut: "Ctrl+Shift+G",
      action: () => onCommand?.("toggle-git-panel"),
    },
    {
      id: "open-terminal",
      label: t("openTerminal") || "Terminal",
      shortcut: "Ctrl+`",
      action: () => {},
    },
    {
      id: "open-mcp",
      label: "Model Context Protocol (MCP) Hub",
      shortcut: "MCP",
      action: () => onCommand?.("open-mcp"),
    },
    {
      id: "open-settings",
...
export function QuickAccessDialog({
  ...
  onRevealFolder,
  onCommand,
}: QuickAccessDialogProps): React.ReactElement | null {
  const { t } = useI18n();
  ...
  const actions = makeActions(t, onCommand).map((a) => {
  ...
  const commandMatches = useMemo(() => filterCommands(query, t), [query, t]);
          ...
          {showingResults ? (
            <div className={interactiveListClassName("search-popup__results")}>
              {commandMatches.map((cmd, i) => (
                ...
                >
                  <span className="search-popup__cmd-label">
                    {t(cmd.labelKey as any) || cmd.defaultLabel || cmd.id}
                  </span>
