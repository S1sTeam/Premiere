...
const THEME = {
  background: "#1e2638", // Default, will be overridden
  foreground: "#e6e6e6",
  cursor: "#e6e6e6",
  cursorAccent: "#1e2638",
  selectionBackground: "#273248",
  black: "#1e2638",
  brightBlack: "#555555",
  white: "#e6e6e6",
  brightWhite: "#ffffff",
...
export function TerminalPane({ id, visible }: Props): React.ReactElement {
  ...
  const bg = activeVars["--bg-2"] || activeVars["--bg"] || "#1e2638";
  ...
  useEffect(() => {
    ...
    const offData = terminalService.onData((p) => {
      if (p.id === id && typeof p.chunk === "string") term.write(p.chunk);
    });
