  ...
  useEffect(() => {
    ...
    initializeWorkbench().then(async (res) => {
      ...
      if (isBrowserDevPreview) {
        const [projectList, activeProj] = await Promise.all([
          workspaceService.list(),
          workspaceService.active(),
        ]);
        setProjects(projectList);
        const cur = activeProj ?? projectList[0] ?? null;
        if (cur) {
          setActiveProject(cur.id);
          setFolder(cur.path);
        } else {
          setActiveProject(null);
          setFolder(null);
        }
        const ws = await loadChatWorkspace({ restoreLastActive: true });
        setChats(ws.chats);
        setActiveChat(ws.activeChatId);
        if (ws.record) {
          setItems(recordToItems(ws.record));
        } else {
          setItems([]);
        }
        return;
      ...
      if (restoredProvider) {
        setConfig((c) =>
          c
            ? {
                ...c,
                model: restoredProvider.model,
                baseUrl: restoredProvider.baseUrl,
                apiKey: restoredProvider.apiKey ? "***" : "",
      ...
      if (cancelled) return;
      setState({ kind: "fatal", error: String(err) });
    });
    return () => {
      cancelled = true;
    };
