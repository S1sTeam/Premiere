...
function sanitizeLinkUrl(url: string): string {
  const trimmed = url.trim().replace(/[\x00-\x1f\x7f-\x9f]/g, "");
...
function inlineToHtml(text: string): string {
  ...
  while (match !== null) {
    ...
    const [_m, _b1, bold, _i1, italic, _c1, code, _m1, math, _l1, linkText, linkHref, _f1, file, _d1, folder] = match;
    if (bold) {
      result += `<strong>${inlineToHtml(bold)}</strong>`;
    } else if (italic) {
      result += `<em>${inlineToHtml(italic)}</em>`;
    } else if (code) {
      result += `<code>${escapeHtml(code)}</code>`;
    } else if (math) {
      result += `<span class="math-inline">${renderMath(math, false)}</span>`;
    } else if (linkText && linkHref === "#file") {
      result += `<file>${escapeHtml(linkText)}</file>`;
    } else if (linkText && linkHref === "#folder") {
      result += `<folder>${escapeHtml(linkText)}</folder>`;
      ...
      result += `<file>${escapeHtml(file)}</file>`;
    } else if (folder) {
      result += `<folder>${escapeHtml(folder)}</folder>`;
    } else {
