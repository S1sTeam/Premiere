...
const I18nContext = createContext<I18nContextValue>({
  t: ((key: keyof Translations, params?: Record<string, string | number | boolean>) => {
    const actualKey = resolvePluralKey(key, params, "English", languages.English, languages.English);
    let v = languages.English[actualKey as keyof Translations];
    if (v === undefined) v = languages.English[key as keyof Translations];
...
export function I18nProvider({ lang, children }: Props): React.ReactElement {
  ...
  const t = useCallback<TranslateFn>(
    ((key: string, params?: Record<string, string | number | boolean>) => {
      const actualKey = resolvePluralKey(key, params, lang, dict, languages.English);
      let v = dict[actualKey as keyof Translations];
      if (v === undefined) v = languages.English[actualKey as keyof Translations];
