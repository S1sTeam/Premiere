...
fn build_anthropic_body(
    ...
    if let Some(effort_str) = config.reasoning_effort.as_deref() {
        if !effort_str.is_empty() && effort_str != "off" && effort_str != "none" {
            let budget = if let Some(stripped) = effort_str.strip_prefix("budget:") {
                stripped.parse::<u64>().unwrap_or(8192)
            } else {
                match effort_str {
                    "minimal" | "low" => 2048,
                    "medium" | "balanced" => 8192,
                    "high" | "deep" => 16384,
                    "max" | "exhaustive" => 32768,
                    _ => 8192,
                }
            };
            ...
            body["max_tokens"] = serde_json::json!(budget + 8192);
        }
    }
    if !system.is_empty() {
...
fn apply_reasoning_params(config: &LlmConfig, body: &mut serde_json::Value) {
    let raw_effort = match config.reasoning_effort.as_deref() {
        Some(e) if !e.is_empty() && e != "off" && e != "none" => e,
        _ => return,
    };

    let mapped_effort = if let Some(stripped) = raw_effort.strip_prefix("budget:") {
        let n: u64 = stripped.parse().unwrap_or(8192);
        if n <= 2048 {
            "low"
        } else if n <= 8192 {
            "medium"
    ...
    match provider_kind(config) {
        ...
        ReasoningTransport::FlatEffort => {
            body["reasoning_effort"] = serde_json::json!(mapped_effort);
        }
        ReasoningTransport::DeepSeek => {
            ...
            let ds_effort = match mapped_effort {
                "max" | "exhaustive" => "max",
                _ => "high",
            };
