...
The `agent-tool` crate defines and implements built-in tool execution capabilities for the Premire AI agent. It equips the agent with essential workspace interactions, including file system read/write operations, precise patch edits, directory browsing, shell command execution, codebase searching, sub-agent spawning, and Model Context Protocol (MCP) server invocations.
