use workspace_fs::{resolve_safe_path, MAX_FILE_BYTES, MAX_OUTPUT_CHARS};
...
pub async fn tool_read_file(cwd: &str, args: &serde_json::Value) -> Result<String, String> {
    ...
    let limit = args
        ...
        .and_then(|v| v.as_u64())
        .map(|v| v as usize);
    let resolved = resolve_safe_path(cwd, path)?;
...
mod tests {
    ...
    #[tokio::test]
    async fn denies_path_outside_cwd() {
        let dir = tempfile::tempdir().unwrap();
        let args = serde_json::json!({
            "path": "../../outside.txt"
        });
        let err = tool_read_file(dir.path().to_str().unwrap(), &args)
            .await
            .unwrap_err();
