...
impl Agent {
    pub fn build_system_prompt(&self) -> String {
        ...
        if let Some(directive) = crate::prompt::effort_prompt(self.config.reasoning_effort.as_deref()) {
            system.push_str("\n\n");
    ...
    pub fn new(mut config: AgentConfig) -> Self {
        ...
        let mut system = system_prompt(&config.cwd);
        if let Some(directive) = crate::prompt::effort_prompt(config.reasoning_effort.as_deref()) {
            system.push_str("\n\n");
            system.push_str(directive);
        }
        Self {
