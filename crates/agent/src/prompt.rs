...
pub fn system_prompt(cwd: &str) -> String {
    ...
    let _legacy_prompt = if false {
        [
        "You are Premire, an advanced autonomous coding assistant and agent with direct access to the file system and development environment.",
        env_info.as_str(),
        "",
    ...
    let base_prompt = [
        "You are Premire, an autonomous coding agent. Complete software-engineering tasks by inspecting the workspace, making targeted changes, and verifying them with available tools.",
        "<env>",
        env_info.as_str(),
        "</env>",
...
pub fn effort_prompt(effort: Option<&str>) -> Option<&'static str> {
    let e = effort?;
    if e.is_empty() || e == "off" || e == "none" {
        return None;
    }
    if e.starts_with("budget:") {
        return Some("REASONING DIRECTIVE: Execute reasoning calibrated strictly to the allocated token budget. Prioritize high-density technical analysis, verify edge cases, and eliminate speculative assumptions before execution.");
    }
    match e {
        "minimal" => Some("REASONING DIRECTIVE: Minimal effort. Formulate immediate direct execution steps. Minimize deliberation, prioritize concise validation, and proceed directly to action."),
        "low" => Some("REASONING DIRECTIVE: Low effort. Focus on direct implementation logic, quick validation of interfaces, and immediate resolution without unnecessary branch exploration."),
        "medium" | "balanced" => Some("REASONING DIRECTIVE: Balanced effort. Systematically analyze the problem, evaluate key edge cases, verify interface contracts, and execute the optimal solution path."),
        "high" | "deep" => Some("REASONING DIRECTIVE: High effort. Perform deep architectural deliberation, analyze potential failure modes and side effects across dependent modules, rigorously verify correctness, and optimize implementation decisions."),
        "max" | "exhaustive" => Some("REASONING DIRECTIVE: Maximum effort. Conduct exhaustive multi-angle analysis, trace all failure paths, formulate formal proofs of correctness for critical sections, evaluate performance and security boundaries, and ensure complete solution robustness."),
        _ => Some("REASONING DIRECTIVE: Systematically analyze the problem, evaluate key edge cases, verify interface contracts, and execute the optimal solution path."),
    }
}
