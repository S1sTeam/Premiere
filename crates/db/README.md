...
The `db` crate serves as the core database access layer for Premire. Built on SQLite using `rusqlite`, it manages application state persistence, including workspace projects (`Project`), configured LLM providers (`Provider`), custom icons, and database connection pools.
...
fn main() -> Result<(), db::DbError> {
    ...
    let store = ProjectStore::new("premire.db")?;
