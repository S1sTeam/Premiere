...
The `git` crate provides native Git repository integration for Premire. Leveraging `git2` (bindings to `libgit2`), it performs high-performance repository discovery, status inspection, diff generation, commit history querying, and branch operations without spawning external subprocesses.
