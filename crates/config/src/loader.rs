...
pub fn load_config(cwd: &str) -> Config {
    ...
    // Global API key fallback: %APPDATA%/Premire/.env (fallback: OpenVibe/.env)
    if let Some(data_dir) = dirs::data_dir() {
        let premire_env = data_dir.join("Premire").join(".env");
        if premire_env.exists() {
            dotenv::load_dotenv(&premire_env, &mut env);
        } else {
            let legacy_env = data_dir.join("OpenVibe").join(".env");
            dotenv::load_dotenv(&legacy_env, &mut env);
        }
    }
    ...
    if base_url.is_empty() {
        base_url = "https://free.sysik.mom/v1".to_string();
    }
    if model.is_empty() {
        model = "sonnet-5".to_string();
    }
    if provider_id.is_none() {
        provider_id = Some("premire".to_string());
    }
