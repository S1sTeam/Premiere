...
pub fn resolve_config_path<P: AsRef<Path>>(
    ...
    let project_premire = cwd_path.as_ref().join("premire.toml");
    if project_premire.exists() {
        return project_premire;
    }
    let project_config = cwd_path.as_ref().join("openvibe.toml");
    if project_config.exists() {
        return project_config;
    }
    let global_premire = global_config_dir.as_ref().join("premire.toml");
    if global_premire.exists() {
        return global_premire;
    }
