...
<p align="center">
  <a href="https://github.com/s1steam/premire">GitHub</a> ·
  <a href="https://t.me/sysgood">Telegram</a> ·
  <a href="mailto:kloxaisupport@gmail.com">kloxaisupport@gmail.com</a> ·
...
<p align="center">
  <a href="https://github.com/s1steam/premire/actions"><img src="https://img.shields.io/github/actions/workflow/status/s1steam/premire/.github/workflows/build.yml?style=flat-square&logo=githubactions&label=build" alt="CI" /></a>
...
- **`crates/mcp`**：Stdio 傳輸的 MCP 客戶端（JSON-RPC 2.0）。控制 MCP 伺服器程序生命週期、設定檔解析（`premire.toml`）、工具發現（`tools/list`）、執行分派（`tools/call`）、連線健康追蹤和程序恢復。
...
- **設定檔**：透過 UI 設定或直接在工作區根目錄的 `premire.toml` 中宣告來進行配置。
...
```bash
git clone https://github.com/s1steam/premire.git
cd premire
npm install
```
