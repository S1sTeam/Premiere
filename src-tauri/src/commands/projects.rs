...
#[tauri::command]
pub async fn projects_pick_folder(app_handle: tauri::AppHandle) -> Result<Option<String>, String> {
...
#[tauri::command]
pub fn projects_open_path(state: State<AppState>, path: String) -> Result<Option<Project>, String> {
    let mut store = state.projects.lock().map_err(|e| e.to_string())?;
    let project = store.add(&path).map_err(|e| e.to_string())?;
