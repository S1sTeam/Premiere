...
fn setup_tray(app: &tauri::App) -> Result<(), Box<dyn std::error::Error>> {
    let show_item = MenuItemBuilder::with_id("show", "Open Premire").build(app)?;
    let quit_item = MenuItemBuilder::with_id("quit", "Quit Premire").build(app)?;
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let base_data = dirs::data_dir().unwrap_or_else(|| std::path::PathBuf::from("."));
            let premire_dir = base_data.join("Premire");
            let app_dir = if premire_dir.exists() {
                premire_dir
            } else {
                let legacy_dir = base_data.join("OpenVibe");
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::projects::projects_open_path,
